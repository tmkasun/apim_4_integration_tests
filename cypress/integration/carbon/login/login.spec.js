describe("Basic login to carbon console", () => {
    ...
    it.skip("Create developer portal user", () => {
        cy.url().should('contains', 'https://localhost:9443/carbon/admin/index.jsp');
        ...
        cy.url().should('contains', 'https://localhost:9443/carbon/user/add-step1.jsp');
        ...
        cy.url().should('contains', 'https://localhost:9443/carbon/user/add-step2.jsp');
        cy.get('#edit_users input[type=checkbox][value="Internal/subscriber"]').click();
        cy.get('#edit_users').submit();
        cy.url().should('contains', 'https://localhost:9443/carbon/user/user-mgt.jsp');
    ...
    it("Delete users", () => {
        ...
        while (i > 0) {
            cy.visit('https://localhost:9443/carbon/admin/index.jsp');
            cy.get('.normal:nth-child(3) .normal:nth-child(2) li:nth-child(2) > .menu-default').click();
            cy.url().should('contains', 'https://localhost:9443/carbon/userstore/index.jsp');
            cy.get('.tableEvenRow .icon-link').click();
            cy.url().should('contains', 'https://localhost:9443/carbon/user/user-mgt.jsp');
